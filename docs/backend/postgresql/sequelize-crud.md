...
**WIP**
Looking in details , the following fields are returned in addition to the attributes that we've defined in the model and the information we passing in.
- **ToDo**: Update "Sequelize Basics" (Model Notes) to reflect Model.sync() without Primary Key vs with Primary Key definition
- When a Primary Key is not define for a model (we should have define it), the id field is auto-generated by Sequelize and is a primary key of the collection. Its value is a unique identifier for the database record.
- **ToDo** Does Sequelize v6 supports Optimistic Locking?
...
The returned result will be similar to what was displayed when we created the record.

It is an instance of Sequelize Model.
