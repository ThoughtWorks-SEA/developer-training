...
In the model that we have defined previously, the database table is synchronised whenever the model is imported, ie. Database table could be dropped whenever it's imported.

We need to some refactoring to remove the model dependency on database connection instance. We could then import the `SimplePokemon` model into our preferred javascript file(s) eg. "index.js", "create.js", without worry of the database connectivity dependencies.

1. Replacing `db/index.js` with a boilerplate tool that could be generated by the `sequelize-cli` migration tools. We will talk about the tool next time.

2. Remove the database synchronisation code from `simple-pokemon.model.js`.
...
const SimplePokemonModel =  require('./db/index.js');

// db.sequelize.sync(); // production code: do nothing if we have already created tables from migration window
db.sequelize.sync({force: true}); // development purpose

// Use SimplePokemonModel to do your database query.
```
